...
from evonorm import EvoNormB0, EvoNormS0
...
def _evonorm(channels, version):
    if version in ['b0', 'B0']:
        return EvoNormB0(channels, training=True)
    elif version in ['s0', 'S0']:
        return EvoNormS0(channels, groups=4)
...
class CIFARBasicBlockV1(HybridBlock):
    def __init__(self, channels, stride, downsample=False, in_channels=0,
        ...
        if evonorm is None:
            ...
            self.body.add(_evonorm(channels, evonorm))
...
class CIFARBasicBlockV2(HybridBlock):
    def __init__(self, channels, stride, downsample=False, in_channels=0,
        ...
        if evonorm is None:
            ...
            self.body.add(_evonorm(in_channels, evonorm))
        self.body.add(_conv3x3(channels, stride, in_channels))
        if evonorm is None:
            self.body.add(norm_layer(**({} if norm_kwargs is None else norm_kwargs)))
            self.body.add(nn.Activation('relu'))
    ...
    def __init__(self, block, layers, channels, classes=10,
        ...
        with self.name_scope():
            ...
            if evonorm is None:
                ...
                self.features.add(_evonorm(in_channels, evonorm))
            self.features.add(nn.GlobalAvgPool2D())
